package com.yxedu.earth.utils.json.jackson;
...
public class JacksonProvider implements JsonProvider {
  ...
  @Override
  public <T> T parse(String text, TypeReference valueTypeRef) {
    try {
  ...
  @Override
  public String getFieldAsText(Object bean, String field) throws Exception {
    ...
    rootNode.fields().forEachRemaining(entry -> queue.offer(entry));
    ...
    while ((entry = queue.poll()) != null) {
      ...
      if (entry.getValue().isContainerNode()) { // recursively
        entry.getValue().fields().forEachRemaining(e -> queue.offer(e));
      }
    }
    return null;
  }
